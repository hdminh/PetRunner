...
async function buildWindows(paths) {
  if (!commandExists("cmake", ["--version"]) || !commandExists("ninja", ["--version"]) || !process.env.VCPKG_ROOT) {
    throw new Error("Windows source builds require Visual Studio 2022 Build Tools (Desktop C++), CMake, Ninja, and VCPKG_ROOT: https://learn.microsoft.com/vcpkg/get_started/get-started-vs");
  }

  await rm(paths.app, { recursive: true, force: true });
  await mkdir(paths.app, { recursive: true });
  const build = path.join(paths.root, "build", PACKAGE.version);
  await rm(build, { recursive: true, force: true });
  run("cmake", ["-S", path.join(paths.source, "windows", "native"), "-B", build, "-G", "Ninja", "-DCMAKE_BUILD_TYPE=Release", `-DCMAKE_TOOLCHAIN_FILE=${path.join(process.env.VCPKG_ROOT, "scripts", "buildsystems", "vcpkg.cmake")}`, "-DVCPKG_TARGET_TRIPLET=x64-windows-static"], { cwd: paths.source });
  run("cmake", ["--build", build], { cwd: paths.source });
  await cp(path.join(build, "bin", "PetRunner.exe"), paths.executable);
}
