...
export async function install({ force = false, terminate = terminateRunningApp } = {}) {
  ...
  if (!force && installed?.version === PACKAGE.version && installed?.buildStamp !== buildStamp) {
    console.log(`Local sources changed since the last install; rebuilding PetRunner ${PACKAGE.version}...`);
  } else {
...
Usage:
  npx @hdminh/pet-runner start [--pets-dir PATH] [--force] [--yes] [--setup]
  npx @hdminh/pet-runner install [--force] [--yes] [--setup]
  npx @hdminh/pet-runner setup
  npx @hdminh/pet-runner update
  npx @hdminh/pet-runner uninstall

Interactive setup (TTY) asks for pets directory, selected pet, Agent Monitor
provider (macOS), usage/analytics providers, autonomy, and menu bar visibility.
Use --yes / --no-setup to skip, or --setup to force the wizard again.`);
}
  ...
  switch (options.command) {
    ...
    case "setup":
      ...
      if (existsSync(resolveInstallPaths().executable)) {
        ...
        console.log("Setup applied to the installed PetRunner.");
      } else {
        console.log("Setup saved. Run start/install to build PetRunner and apply it.");
      }
      break;
    case "update": {
